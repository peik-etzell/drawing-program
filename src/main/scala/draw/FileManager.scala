...
object FileManager {
    ...
    val fileMenu: Menu = new Menu("Open") {
            ...
            contents = new BoxPanel(scala.swing.Orientation.Horizontal) {
                ...
                contents += new Button(Action("Save") {
                    if (!textField.text.isBlank()) {
                        ...
                        write(file)
                        fileMenu.contents += new MenuItem(Action(fileName) {
                            Canvas.open(read(file))
                        }) 
                        close()
                    }
    ...
    // For testing purposes, does the same as pushing buttons
    def save(file: File) = { 
        Canvas.open(read(file))
    }
    ...
    def read(file: File): Buffer[Element] = {
